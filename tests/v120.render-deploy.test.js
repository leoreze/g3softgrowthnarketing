...
test('v1.0.20 binds the web server to 0.0.0.0 for Render',()=>{
  const source=fs.readFileSync(path.join(root,'src/server.js'),'utf8');
  assert.match(source,/app\.listen\(env\.port,'0\.0\.0\.0'/);
});

test('v1.0.20 Render Blueprint uses pre-deploy migrations and CI-gated deploys',()=>{
...
test('v1.0.20 keeps production secrets out of the Blueprint',()=>{
  const yaml=fs.readFileSync(path.join(root,'render.yaml'),'utf8');
