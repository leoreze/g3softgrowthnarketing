...
test('v1.0.0 migration history remains append-only',()=>{const files=fs.readdirSync(path.join(root,'src/db/migrations')).filter(f=>f.endsWith('.sql')).sort();assert.ok(files.indexOf('011_automation_notifications.sql')>=0);assert.equal(files.at(-1),'030_relationship_center_operational.sql');assert.equal(files.length,27);assert.ok(files.includes('014_task_execution_center.sql'));assert.ok(files.includes('013_task_execution_deliverables.sql'))});
