...
test('v1.0.1 production roadmap migration is additive and append-only', () => {
  ...
  const migrations=fs.readdirSync(path.join(root,'src/db/migrations')).filter(f => f.endsWith('.sql')).sort(); assert.ok(migrations.includes('013_task_execution_deliverables.sql')); assert.equal(migrations.at(-1), '030_relationship_center_operational.sql'); assert.ok(migrations.includes('014_task_execution_center.sql'));
});
